...
if(size) {
  for(int i = 0; i < cantidad; i++)
  {
    for(int j = 0; j < rowMatrix; j++)
    {
      ...
      {
       case '-':
         if(j == 0) { // Posición en el arreglo de numbers
         drawRows(rowMatrix, cantidad * colMatrix, 0,
                 (i * colMatrix), matrix, size);
         } else if(j == 3) { // Posición en el arreglo de numbers
         drawRows(rowMatrix, cantidad * colMatrix, size + 1,
                 (i * colMatrix), matrix, size);
         } else if(j == 6) { // Posición en el arreglo de numbers
         drawRows(rowMatrix, cantidad * colMatrix, (size*2) + 2,
                 (i * colMatrix), matrix, size);
         }
         break;
       case '|':
         if(j == 1) { // Posición en el arreglo de numbers
         drawCols(rowMatrix, cantidad * colMatrix, 0,
                 (i * colMatrix), matrix, size);
         } else if(j == 2) { // Posición en el arreglo de numbers
         drawCols(rowMatrix, cantidad * colMatrix, 0,
                 (colMatrix-1) + (i * colMatrix), matrix, size);
         } else if(j == 4) { // Posición en el arreglo de numbers
         drawCols(rowMatrix, cantidad * colMatrix, size + 1,
                 (i * colMatrix), matrix, size);
         } else if(j == 5) { // Posición en el arreglo de numbers
         drawCols(rowMatrix, cantidad * colMatrix, size + 1,
                 (colMatrix-1) + (i * colMatrix), matrix, size);
         }
         break;
